    ...
    <form action="index.php" method="GET" x-show="searchOpen"
        ...
        <div class="px-4 py-8">
            ...
            <input x-ref="student_name" autofocus x-init="fetchStudent($el.value)" @focus.once="fetchStudent($el.value)"
                ...
                class="border w-full px-3 py-2 border-gray-300 focus:ring-2 focus:ring-neutral-800 focus:border-neutral-800" name="student_name">

            <template x-if="message.length">
                <ol class="flex flex-col mt-4 gap-y-2 w-full">
                    <template x-for="(student, index) in message" :key="student.student_id">
                        <button type="button"
                            ...
                            class="p-3 border border-gray-300 bg-white hover:bg-gray-50 text-left cursor-pointer focus:ring-2 focus:ring-neutral-800">
