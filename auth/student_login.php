...
if (!empty($_POST['name'])) {
  ...
  $stmt = $connection->prepare("select * from users where name = ? and role = 'student'");
...
<body class="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
  <div class="max-w-md w-full space-y-8">
    ...
    <form class="mt-8 space-y-6" action="student_login.php" method="POST">
      <div class="bg-white py-8 px-6 rounded-lg space-y-4">
        ...
        <div>
          <button
            type="submit"
            class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-neutral-800 hover:bg-neutral-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-800 transition-colors duration-200">
            Sign in
          </button>
        </div>
