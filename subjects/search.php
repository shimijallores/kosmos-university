 <div x-cloak x-show="open" x-transition.opacity.duration.200ms x-on:keydown.esc.window="open = false"
   ...
   <!-- Modal Dialog -->
   <form action="index.php" method="GET" x-show="open"
     ...
     <div class="px-4 py-8">
       ...
       <input @input.debounce.750="fetchStudent($el.value)" type="text" id="student_name" class="border w-full border-neutral-200 px-2 py-1 rounded" name="student_name">
       ...
       <template x-if="message.length">
         <ol class="flex flex-col mt-2 gap-y-1 justify-start w-full">
           <template x-for="(student, index) in message" :key="student.student_id">
             <button type="button" @click="studentNumber = student.student_number; open = false" class="p-1 border border-neutral-200 hover:opacity-80 cursor-pointer">
